...
function reflowLines(m, row, col, content) {
    ...
    buster.log("Reflow", h(content), "at", width, h(S.fit(content, width)));
...
module.exports = {
    ...
    insertRow: function (index, columns) {
        ...
        this.columns = initColumns(this.columns.length);
        this.redraw();
    },
    ...
    append: function (row, col, content) {
        ...
        buster.log(h(this.rows[row][col] + content), lines);
        this.redraw();
    },
